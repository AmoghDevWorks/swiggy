            ...
            <div className="w-full h-full flex flex-wrap">
                {
                    listOfRestaurant.map(restaurant=>(
                        ...
                        <Link className="bg-slate-100 text-center m-1 p-1.5 h-auto w-56 hover:border-solid hover:border-2 hover:border-black" key={restaurant.info.id} to={"/restaurants/"+restaurant.info.id}><RestaurantCard resdata={restaurant} /></Link>
                    ))
                }
            </div>
        </div>
